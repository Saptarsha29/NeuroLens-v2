        ...
        <p className="text-slate-400 mt-1">
          ...
          {!currentUser && (
            <span className="text-yellow-600">
              You are not signed in — results will not be saved.
            </span>
          )}
        </p>
      </div>
          ...
          return (
            <div
              ...
              className={`flex-1 text-center p-3 rounded-lg border text-sm font-medium transition-colors
                ${done ? 'bg-emerald-900/30 border-green-300 text-emerald-400' : 'bg-slate-900/50 border-slate-700/50 text-slate-500'}`}
            >
              <span className="text-xl">{icon}</span>
              <p className="mt-1">{label}</p>
              <p className="text-xs">{done ? '✅ Done' : 'Pending'}</p>
            </div>
          )
        })}
      </div>
